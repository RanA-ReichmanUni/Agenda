...
export default function DemoAgendaPage() {
  ...
  const { getAgenda, createArticle, deleteArticle } = useDemo();
  ...
  useEffect(() => {
    const fetchAgendaAndArticles = async () => {
      setLoading(true);
      try {
        const agendaData = getAgenda(agendaId);
        if (!agendaData) throw new Error("Failed to fetch agenda");

        setAgenda({ title: agendaData.title, createdAt: agendaData.created_at });
        setArticles(agendaData.articles);
        setError(null);
      } catch (err: any) {
  ...
  const handleAddArticle = async (newArticle: Omit<Article, "id">) => {
    try {
      const added = await createArticle(agendaId, newArticle);
    ...
    try {
      ...
      const restored = await createArticle(agendaId, rest);
  ...
  const handleRemoveArticle = async (articleId: number) => {
    try {
      ...
      const articleToRestore = articles.find(a => Number(a.id) === articleId || a.id === articleId.toString());
      //setArticleToDelete(articleToRestore || null);
      if (!articleToRestore) {
        throw new Error("Article not found");
      }

      await deleteArticle(agendaId, articleId);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-yellow-50 to-black-50 pt-0 pb-2 px-2 md:px-0">
      ...
      <style>{`
        ...
        @keyframes agenda-header { 0% { opacity: 0; transform: translateY(-50px) rotate(-2deg); } 100% { opacity: 1; transform: translateY(0) rotate(0); } }
        .animate-agenda-header { animation: agenda-header 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) both; }
        @keyframes agenda-title { 0% { opacity: 0; transform: scale(0.8) translateX(-20px); } 100% { opacity: 1; transform: scale(1) translateX(0); } }
        .animate-agenda-title { animation: agenda-title 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) 0.2s both; }
        @keyframes agenda-date { 0% { opacity: 0; transform: translateY(20px); } 100% { opacity: 1; transform: translateY(0); } }
        .animate-agenda-date { animation: agenda-date 0.6s ease-out 0.4s both; }
        @keyframes form-float { 0% { opacity: 0; transform: translateY(30px) scale(0.95); } 100% { opacity: 1; transform: translateY(0) scale(1); } }
        .animate-form-float { animation: form-float 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) 0.6s both; }
        @keyframes articles-container { 0% { opacity: 0; } 100% { opacity: 1; } }
        .animate-articles-container { animation: articles-container 0.6s ease-out 0.8s both; }
        @keyframes section-title { 0% { opacity: 0; transform: translateX(-20px); } 100% { opacity: 1; transform: translateX(0); } }
        .animate-section-title { animation: section-title 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) 0.9s both; }
        @keyframes article-card { 0% { opacity: 0; transform: translateY(20px) scale(0.95); } 100% { opacity: 1; transform: translateY(0) scale(1); } }
        .animate-article-card { animation: article-card 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) both; }
        @keyframes empty-state { 0% { opacity: 0; transform: scale(0.9); } 100% { opacity: 1; transform: scale(1); } }
        .animate-empty-state { animation: empty-state 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) 1s both; }
      `}</style>
    </div>
  );
}
